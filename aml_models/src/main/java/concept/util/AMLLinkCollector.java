...
public class AMLLinkCollector {
	...
	private static CAEXFileType query, data;
	private static Set<InternalLinkType> uniqueLinks = new HashSet<InternalLinkType>();
	private static boolean initialized;
	
	public AMLLinkCollector(CAEXFileType query, CAEXFileType data) {
		AMLLinkCollector.query = query;
		AMLLinkCollector.data = data;
		initialized = false;
	}
	
	public void init () {
		if(uniqueLinks.isEmpty()) {
			List<InternalLinkType> links = AMLHelperFunctions.getAllLinks(query);		
	...
	public static Set<EILinkRefSide> getPartnersOfEI (ExternalInterfaceType ei){
		...
		SystemUnitClassType suc = AMLHelperFunctions.getParentOfEI(ei, query);
		if(suc == null)
			return partners;
		
		else{
			EILinkRefSide self = new EILinkRefSide(suc.getID(), ei.getName());
//			self.setEIId(ei.getID());
...
//		self.setEIId(AMLHelperFunctions.);
		return getPartnersOfEI(self);
	}	
		...
		if(links != null) {
			for(InternalLinkType link : links) {
				...
				if(partner.getType() == null) {
					...
					if(getEIIdFromFile(partner, query) == null) {
						partner.setType(EndpointType.DATA);
						partner.setEIId(getEIIdFromFile(partner, data));
					}
					else
	...
	public static Set<InternalLinkType> getLinksOfEI (ExternalInterfaceType ei) {
		...
		SystemUnitClassType parent = AMLHelperFunctions.getParentOfEI(ei, query);
